...
def draw_callback_2d(self, context):
    ...
    for i in range(asset_count):
        ...
        if pos_x + thumb > x + handle + ui.asset_container_width - handle:
            paint.rect_2d(pos_x, y, space_left, thumb, normal)
            paint.image(
        ...
        paint.image(asset.thumbnail, pos_x, y, thumb, thumb)
        for _ in range(3):
            paint.text(asset.name, pos_x+5, y+7, int(thumb/6), color=(0,0,0,0.25))
        paint.text(asset.name, pos_x+5, y+7, int(thumb/6), color=(1,1,1,0.25))
    ...
    and asset_count < len(assets)+1: 
        ...
        paint.text("▼ ▼ ▼", pos_x+space_left+handle-5, y+5, handle-6, rotation=90)
    ...
    and int(ui.hover_on) < len(ui.library["filtered"]):
        ...
        paint.image(asset.thumbnail, hover_x-dif, y-dif, thumb_scale, thumb_scale)
        for _ in range(3):
            paint.text(asset.name, hover_x+5, y+7, int(thumb_scale/6), color=(0,0,0,1))
        paint.text(asset.name, hover_x+5, y+7, int(thumb_scale/6))
    ...
    paint.text("<" if ui.open else ">", x+handle/3, y+h-15, handle-6)
    paint.text("Asset", x+handle-5, y+5, handle-6, rotation=90)
...
def draw_callback_3d(self, context):
    ...
    if ui.drag and ui.active_asset_index >= 0 :
        ...
        if asset.type in ["Model"]:
            paint.draw_bbox(ui.bbox_location, ui.bbox_rotation, ui.bbox_min, ui.bbox_max)
